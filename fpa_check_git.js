...
function _dmYearStatusBadges() {
  const aLabel = actualsYear || '—';
  const pLabel = planYear    || '—';
  return (
...
function _refreshTopBarYearPills() {
  let pill = document.getElementById('_topBarYrPill');
  if (!pill) {
  ...
  pill.innerHTML =
    `<span style="font-size:9px;padding:2px 7px;border-radius:10px;` +
      ...
      `title="Active actuals year">📊 ${actualsYear||'—'}</span>` +
    `<span style="font-size:9px;padding:2px 7px;border-radius:10px;` +
      `background:rgba(167,139,250,.15);color:#a78bfa;font-weight:700;white-space:nowrap" ` +
      `title="Active plan year (${_aopCode()})">📋 ${planYear||'—'}</span>`;
}
